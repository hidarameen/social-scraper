...
export function TaskForm({ task, onSuccess }: TaskFormProps) {
  ...
  const form = useForm<InsertTask>({
    ...
    defaultValues: {
      ...
      scrapeMethod: (task as any)?.scrapeMethod || "html",
      status: (task as any)?.status || "active",
      messageTemplate: (task as any)?.messageTemplate || "<b>[ScrapeMaster]</b>\nAccount: {account}\nPlatform: {platform}\nDate: {date}\n\n{text}\n\n<a href=\"{url}\">View Post</a>",
      includeImages: (task as any)?.includeImages ?? true,
      includeVideos: (task as any)?.includeVideos ?? true,
      aiEnabled: (task as any)?.aiEnabled ?? false,
      aiProvider: (task as any)?.aiProvider || "openai",
      aiModel: (task as any)?.aiModel || "gpt-4o-mini",
      aiPrompt: (task as any)?.aiPrompt || "",
    },
  });
